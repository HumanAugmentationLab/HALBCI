...
myimgfile = ['C:\Users\gsteelman\Desktop\SummerResearch\HALBCI\SandBox\Media\left.png'];
fprintf('Using image ''%s''\n', myimgfile);
imdata=imread(myimgfile);
imagetexLeft=Screen('MakeTexture', w, imdata);

myimgfile = ['C:\Users\gsteelman\Desktop\SummerResearch\HALBCI\SandBox\Media\right.png'];
fprintf('Using image ''%s''\n', myimgfile);
imdata=imread(myimgfile);
...
try
    ...
    for i = 1:numTrials
        ...
        Screen('DrawTexture', w, imagetexs(mod(orderList(i),2)+1));
        mrk = (mod(orderList(i),2)+1)*100;%left = 100 , right = 200
        ...
        mrk = (mod(orderList(i),2)+1)*100 + 1%left = 101 right = 201
        efficientChoice(w,wRect,Hz,Hz2,Trialslength,imagetexs(mod(orderList(i),2)+1),outlet,mrk);
