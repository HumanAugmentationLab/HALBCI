...
myimgfile = ['C:\Users\alakmazaheri\Documents\BCI\HALBCI\SandBox\Media\openEyes.jpg'];
fprintf('Using image ''%s''\n', myimgfile);
imdata=imread(myimgfile);
imagetexOpen=Screen('MakeTexture', w, imdata);

myimgfile = ['C:\Users\alakmazaheri\Documents\BCI\HALBCI\SandBox\Media\closedEyes.jpg'];
fprintf('Using image ''%s''\n', myimgfile);
imdata=imread(myimgfile);
...
[pahandle,wavedata] = loadSound(['C:\Users\alakmazaheri\Documents\BCI\HALBCI\SandBox\Media\default_ding.wav']);

%This part is just a buffer screen to wait for the user to click when ready
try
    ...
    %situated
    Screen('FillRect',w, black);
    Screen('DrawText', w, 'Click mouse to start session', wW/2-100, wH/2, white);
