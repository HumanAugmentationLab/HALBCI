...
experimentName = 'experiment_log25.txt';      % Log file name

% Duration
trialLength = 60.1;                  % Trial length (s)  --- always add 100 ms for buffer
numTrials = 8;                      % Number of trials per run - must be even number
...
scalingCoeff = 1;               % Fix bug of speed dependening on display size

% Checkerboard Display
Hz = [15 15];                         % Frequencies to display
...
color2Transp = 250; %150                   % Transparency (0: transparent, 250: opaque)
...
orderedDisplay = zeros(1, numTrials);
...
try
    ...
    for n = 1:numTrials
        ...
        rightFreq = leftFreq; %Hz(Hz ~= targetFreqs(n));
        ...
        while toc(start) - startOffset < trialLength
            ...
            % timestamps
            [vbl StimulusOnsetTime FlipTimestamp Missed Beampos]  = Screen('Flip', window, vbl + (waitframes-buffer) * ifi, 0, 1);
