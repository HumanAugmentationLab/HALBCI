...
% 3/4 covert (display: 0) and 1/4 overt (display: 1)
orderedDisplay = [zeros(1, 0.75*halfSize) ones(1, 0.25*halfSize) zeros(1, 0.75*halfSize) ones(1, 0.25*halfSize)];                % Checker v. video trial
...
try
    ...
    dstRectC = CenterRectOnPointd(dispRect, xCenter, yCenter);
    ...
    for n = 1:numTrials
        ...
        if displayType == 0
            if targetFreqs(n) == min(targetFreqs)
                % Condition 1: COVERT LOW
                mCondition = mCondition1;
            else
                % Condition 2: COVERT HIGH
                mCondition = mCondition2;
            end
        else
            if targetFreqs(n) == min(targetFreqs)
                % Condition 3: OVERT LOW
                mCondition = mCondition3;
            else
                % Condition 4: OVERT HIGH
