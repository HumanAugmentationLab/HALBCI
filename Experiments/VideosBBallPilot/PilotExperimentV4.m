...
% Still have an issue of the checkerboard freqs not matching the values
% assigned when running the video
% When just checkerboard then 30 hz gives spikes at 15, 30, etc
% when videos at 30, then spikes at 10 , 20, 30 etc
% when videos at 6 ff = 2.7
% Going to try changing to be based on timeinstead of number of cycles
...
color1Transp = 175;                 % Transparency (0: transparent, 250: opaque)
color2Transp = 50;                  % Transparency (0: transparent, 250: opaque)
...
orderedDisplay = [zeros(1, halfSize/2) ones(1, halfSize/2) zeros(1, halfSize/2) ones(1, halfSize/2)];                % Checker v. video trial
% orderedDisplay = ones(1, numTrials);
    ...
    for n = 1:numTrials
        ...
        if displayType == 0
            if targetFreqs(n) == min(targetFreqs)
                % Condition A: CHECK LOW
                mCondition = mConditionA;
            else
                % Condition B: CHECK HIGH
                mCondition = mConditionB;
            end
        else
            if targetFreqs(n) == min(targetFreqs)
                % Condition C: VIDEO LOW
                mCondition = mConditionC;
            else
                % Condition D: VIDEO HIGH
